...
class TexVector {
   public:
    explicit TexVector(int initSize = 0)
        : theSize{initSize}, theCapacity{initSize + SPARE_CAPACITY} {
        objects = new Object[theCapacity];
    }

    TexVector(const TexVector &rhs)
        : theSize{rhs.theSize}, theCapacity{rhs.theCapacity}, objects{nullptr} {
        objects = new Object[theCapacity];
        for (int k = 0; k < theSize; ++k) objects[k] = rhs.objects[k];
    }

    TexVector &operator=(const TexVector &rhs) {
        TexVector copy = rhs;
        std::swap(*this, copy);
        return *this;
    }

    ~TexVector() { delete[] objects; }

    TexVector(TexVector &&rhs)
        : theSize{rhs.theSize},
    ...
    TexVector &operator=(TexVector &&rhs) {
